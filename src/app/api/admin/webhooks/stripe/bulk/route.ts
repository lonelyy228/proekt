...
import { adminWebhookBulkReplaySchema } from "@/server/validators/admin";
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const parsed = adminWebhookBulkReplaySchema.parse(body);
    const result = await adminService.replayWebhookEventsBulk(user.id, parsed);
