...
import { adminWebhookExportQuerySchema } from "@/server/validators/admin";
...
const buildExportFilename = (): string => {
  ...
  return `stripe_webhooks_export_${stamp}.csv`;
};
...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const parsed = adminWebhookExportQuerySchema.parse({
    ...
    const result = await adminService.exportWebhookEventsCsv(user.id, parsed);
