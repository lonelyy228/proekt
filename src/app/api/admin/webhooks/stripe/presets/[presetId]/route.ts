...
import {
  adminWebhookFilterPresetIdParamsSchema,
  adminWebhookFilterPresetUpdateSchema
...
export async function PATCH(
  ...
  try {
    ...
    const params = adminWebhookFilterPresetIdParamsSchema.parse(context.params);
    const body = await request.json();
    const parsed = adminWebhookFilterPresetUpdateSchema.parse(body);
    const result = await adminWebhookPresetsService.update(user.id, params.presetId, parsed);
...
export async function DELETE(
  ...
  try {
    ...
    const params = adminWebhookFilterPresetIdParamsSchema.parse(context.params);
    const result = await adminWebhookPresetsService.delete(user.id, params.presetId);
    return apiSuccess(result, requestId);
  } catch (error: unknown) {
    return apiError(error, requestId);
  }
