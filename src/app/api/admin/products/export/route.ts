...
import { adminProductsExportQuerySchema } from "@/server/validators/admin";
...
const buildExportFilename = (): string => {
  ...
  return `products_export_${stamp}.csv`;
};
...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const parsed = adminProductsExportQuerySchema.parse({
    ...
    const result = await adminService.exportProductsCsv(user.id, parsed);
