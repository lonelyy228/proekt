...
import { adminProductFilterPresetCreateSchema } from "@/server/validators/admin";
...
export async function GET(): Promise<Response> {
  ...
  try {
    ...
    const result = await adminService.listProductFilterPresets(user.id);
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const parsed = adminProductFilterPresetCreateSchema.parse(body);
    const result = await adminService.createProductFilterPreset(user.id, parsed);
    return apiSuccess(result, requestId);
  } catch (error: unknown) {
    return apiError(error, requestId);
  }
