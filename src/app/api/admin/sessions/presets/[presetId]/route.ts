...
import {
  adminSessionFilterPresetIdParamsSchema,
  adminSessionFilterPresetUpdateSchema
...
export async function PATCH(
  ...
  try {
    ...
    const params = adminSessionFilterPresetIdParamsSchema.parse(context.params);
    const body = await request.json();
    const parsed = adminSessionFilterPresetUpdateSchema.parse(body);
    const result = await adminSessionPresetsService.update(user.id, params.presetId, parsed);
...
export async function DELETE(
  ...
  try {
    ...
    const params = adminSessionFilterPresetIdParamsSchema.parse(context.params);
    const result = await adminSessionPresetsService.delete(user.id, params.presetId);
    return apiSuccess(result, requestId);
  } catch (error: unknown) {
    return apiError(error, requestId);
  }
