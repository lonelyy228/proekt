...
import { adminUsersExportQuerySchema } from "@/server/validators/admin";
...
const buildExportFilename = (): string => {
  ...
  return `users_export_${stamp}.csv`;
};
...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const result = await adminService.exportUsersCsv(user.id, parsed);
