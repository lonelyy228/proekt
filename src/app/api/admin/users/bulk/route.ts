...
import { adminUserBulkUpdateSchema } from "@/server/validators/admin";
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const parsed = adminUserBulkUpdateSchema.parse(body);
    const result = await adminService.updateUsersAccessBulk(user.id, parsed);
