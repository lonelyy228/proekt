...
import { adminUserFilterPresetCreateSchema } from "@/server/validators/admin";
...
export async function GET(): Promise<Response> {
  ...
  try {
    ...
    const result = await adminService.listUserFilterPresets(user.id);
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const parsed = adminUserFilterPresetCreateSchema.parse(body);
    const result = await adminService.createUserFilterPreset(user.id, parsed);
    return apiSuccess(result, requestId);
  } catch (error: unknown) {
    return apiError(error, requestId);
  }
