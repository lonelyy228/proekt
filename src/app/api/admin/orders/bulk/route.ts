...
import { adminOrderBulkStatusUpdateSchema } from "@/server/validators/admin";
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const parsed = adminOrderBulkStatusUpdateSchema.parse(body);
    const result = await adminService.updateOrdersStatusBulk(user.id, parsed);
