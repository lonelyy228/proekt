...
import { adminOrderFilterPresetCreateSchema } from "@/server/validators/admin";
...
export async function GET(): Promise<Response> {
  ...
  try {
    ...
    const result = await adminService.listOrderFilterPresets(user.id);
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const parsed = adminOrderFilterPresetCreateSchema.parse(body);
    const result = await adminService.createOrderFilterPreset(user.id, parsed);
    return apiSuccess(result, requestId);
  } catch (error: unknown) {
    return apiError(error, requestId);
  }
