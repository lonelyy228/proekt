...
import { contentBulkStatusUpdateSchema } from "@/server/validators/content";
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const parsed = contentBulkStatusUpdateSchema.parse(body);
    const result = await contentService.updateStatusesBulk(user.id, parsed);
