...
import { adminContentExportQuerySchema } from "@/server/validators/admin";
...
const buildExportFilename = (): string => {
  ...
  return `content_export_${stamp}.csv`;
};
...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const parsed = adminContentExportQuerySchema.parse({
      search: request.nextUrl.searchParams.get("search") ?? undefined,
      status: request.nextUrl.searchParams.get("status") ?? undefined,
      limit: request.nextUrl.searchParams.get("limit") ?? undefined
    });

    const result = await contentService.exportCsv(user.id, parsed);
